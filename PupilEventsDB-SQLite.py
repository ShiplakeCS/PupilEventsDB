...
db = sqlite3.connect('data/pupil-events.db')   # Need to close later with db.close()
cur = db.cursor()
app = Flask(__name__)
...
class EventsDB:
    ...
    __addedStaff = {}
    __addedForms = {}
    __addedYears = {'07':1, '08':2, '09':3, '10':4, '11':5, '12':6, '13':7}
    ...
    def clear_data(self):
        ...
        cur.execute("DELETE FROM EventType")
        cur.execute("DELETE FROM Year")
        cur.execute("DELETE FROM Form")
        cur.execute("DELETE FROM House")
        cur.execute("UPDATE SQLITE_SEQUENCE SET SEQ = 0 WHERE NAME = 'Staff'")
        cur.execute("UPDATE SQLITE_SEQUENCE SET SEQ = 0 WHERE NAME = 'Event'")
        cur.execute("UPDATE SQLITE_SEQUENCE SET SEQ = 0 WHERE NAME = 'Form'")
        db.commit()

    @staticmethod
    def setup_basic_data():

        cur.execute("INSERT INTO House VALUES (1,'Burr'),(2,'Everett'),(3,'Skipwith'),(4,'Orchard'),(5,'Welsh'),(6,'Lower School')")
        cur.execute("INSERT INTO EventType VALUES (1, 'EXC', 'Excellence Slip'), (2, 'INF', 'Infraction')")
        cur.execute("INSERT INTO Year VALUES "
                  "(1, '7'),"
                  "(2, '8'),"
                  "(3, '9'),"
                  "(4, '10'),"
    ...
    def load_XLS_data(self, source_file):
        ...
        for row in range(1, events_data.nrows): # start after the first row to ignore headings
            ...
            # Add staff codes to Staff table as necessary
            if staff_code in self.__addedStaff.keys():
                pass
            else:
                cur.execute("INSERT INTO Staff VALUES (NULL, '{0}', NULL)".format(staff_code))
                self.__addedStaff[staff_code] = cur.lastrowid
                print("Added {0}, id={1}".format(staff_code, self.__addedStaff[staff_code]))

            # Add form code to Form table as necessary
            if form_code in self.__addedForms.keys():
                pass
            else:
                year_id = self.__addedYears[form_code[:2]]
                cur.execute("INSERT INTO Form VALUES (NULL, '{0}', {1})".format(form_code, year_id))
                self.__addedForms[form_code] = cur.lastrowid

            # Add the event to the Event table, choosing the correct type
            if "EXC" in category_code:
                event_type_id = 1
            elif "INF" in category_code or "INC" in category_code:
                event_type_id = 2

            cur.execute("INSERT INTO Event VALUES (NULL, {0}, '{1}', '{2}')".format(event_type_id, self.__addedStaff[staff_code], self.__addedForms[form_code]))
